...
export default function ChildEdit(props) {
  ...
  const handleDelete = (e) => {
    e.preventDefault();
    axios(
      `${process.env.REACT_APP_BASE_URL}/child/deleteChild/${props.location.state.child._id}`,
      {
        method: "DELETE",
  ...
  const handleEdit = (e) => {
    e.preventDefault();
    axios(
      `${process.env.REACT_APP_BASE_URL}/child/updateChild/${props.location.state.child._id}`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        withCredentials: true,
